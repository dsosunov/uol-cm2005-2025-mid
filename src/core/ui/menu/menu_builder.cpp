...
{
  // Use title as value for backward compatibility
  current_->AddChild(std::make_unique<MenuNode>(title, std::move(action), title));
  return *this;
}

MenuBuilder &MenuBuilder::AddLeaf(const std::string &title, const std::string &value, std::unique_ptr<MenuAction> action)
{
...
std::unique_ptr<MenuNode> MenuBuilder::Build() { return std::move(root_); }
