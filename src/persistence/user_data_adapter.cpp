...
namespace persistence
{
    ...
    std::optional<services::UserRecord> UserDataAdapter::FindByEmail(std::string_view email) const
    {
        ...
        ReadWithProcessor(
            [email](const services::UserRecord &user)
            { return user.email == email; },
            [&result](const services::UserRecord &user)
            { result = user; });
        return result;
    }
    std::optional<services::UserRecord> UserDataAdapter::FindByUsername(std::string_view username) const
    {
        std::optional<services::UserRecord> result;
        ReadWithProcessor(
            [username](const services::UserRecord &user)
    ...
    bool UserDataAdapter::ExistsByEmail(std::string_view email) const
    {
        ...
        ReadWithProcessor(
            [email](const services::UserRecord &user)
            { return user.email == email; },
            [&exists](const services::UserRecord &)
            { exists = true; });
        return exists;
    }
    ...
    bool UserDataAdapter::Insert(services::UserRecord &user)
    {
        user.id = GetNextId();
        auto users = LoadAll();
    ...
    bool UserDataAdapter::Update(const services::UserRecord &user)
    {
        auto users = LoadAll();
        bool found = false;
        for (auto &u : users)
    ...
    std::vector<services::UserRecord> UserDataAdapter::LoadAll() const
    {
        ...
        if (IsValid())
        {
            ReadWithProcessor([&users](const services::UserRecord &user)
                              { users.push_back(user); });
        }
        return users;
    }
    bool UserDataAdapter::SaveAll(const std::vector<services::UserRecord> &users) const
    {
        ...
        writer_->ClearBuffer();
        return BaseDataAdapter<services::UserRecord>::WriteAll(*writer_, users, TransformFromUserRecord);
    }
