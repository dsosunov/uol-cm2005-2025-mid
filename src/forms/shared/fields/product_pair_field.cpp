...
{
    CandlestickProductPairField::CandlestickProductPairField(std::set<std::string, std::less<>> allowed_currencies)
        : form::TextField("product_pair", "Currency pair",
                          CurrencyPairBinder<dto::CandlestickQuery>(),
                          std::make_shared<CurrencyPairValidator>(std::move(allowed_currencies))) {}
    template <typename TDTO>
    ProductPairField<TDTO>::ProductPairField(
