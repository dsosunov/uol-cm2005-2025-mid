...
namespace forms::shared
{
    ...
    StartDateField<TDTO>::StartDateField(std::shared_ptr<form::DataSource> data_source, MemberPtr member_ptr)
        : form::SelectionField("start_date", "Select start date", std::move(data_source),
                               form::DateFieldBinder<TDTO>(member_ptr)) {}
    template <typename TDTO>
    EndDateField<TDTO>::EndDateField(std::shared_ptr<form::DataSource> data_source, MemberPtr member_ptr)
