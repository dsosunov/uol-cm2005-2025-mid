...
namespace wallet_forms
{
  ...
  WalletOperationForm::WalletOperationForm(std::shared_ptr<form::FormInputProvider> input_provider,
      ...
      const std::set<std::string, std::less<>> &allowed_currencies)
  {
    ...
    fields.push_back(std::make_shared<CurrencyField>(allowed_currencies));
    fields.push_back(std::make_shared<AmountField>());
    return fields;
  }
