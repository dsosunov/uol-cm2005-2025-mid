...
utils::ServiceResult<services::User> UserLoginAction::ExecuteService(const dto::UserLogin &data, ActionContext &context)
{
  return user_service_->LoginUser(data.username, data.password);
}
void UserLoginAction::DisplayResults(const utils::ServiceResult<services::User> &result, const dto::UserLogin &data, ActionContext &context)
{
