...
std::unique_ptr<MenuNode> BuildMenu(const ServiceContainer &container)
{
  ...
  return MenuBuilder("Trading Platform")
      ...
      .AddLeaf("Take away", std::make_unique<WalletWithdrawAction>(wallet_service, trading_service))
      .Parent()
      .AddBranch("Transactions")
      .AddLeaf("Show last 5", std::make_unique<TransactionShowLast5Action>(transactions_service))
      .AddLeaf("Show by product pair",
               std::make_unique<TransactionShowByPairAction>(transactions_service, trading_service))
      .AddLeaf("Show activity summary",
               std::make_unique<TransactionActivitySummaryAction>(transactions_service, trading_service))
      .Parent()
