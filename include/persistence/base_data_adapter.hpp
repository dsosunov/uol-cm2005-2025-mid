...
namespace persistence
{
    ...
    {
    public:
        explicit BaseDataAdapter(std::shared_ptr<data::CsvReader> reader)
        ...
        void ReadWithProcessor(TPredicate predicate, std::function<void(const TEntity &)> processor) const
        {
            if (!IsValid())
            {
                return;
            }
            reader_->ReadWithProcessor([this, &predicate, &processor](const data::CsvRecord &csv_record)
                                       {
                auto entity = TransformToEntity(csv_record);
